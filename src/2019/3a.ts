...
const drawLine = (grid: Grid<number>, from: Coord, step: Step) => {
    const distanceBefore = grid.forCoord(from);
    let curr = from;
    for (let i = 1; i <= step.l; i++) {
        curr = nextPos(curr, step.d);
        grid.set(curr, distanceBefore + i);
    }
    return curr;
};
...
const rig = new Rig(3,
    async (d) => {
        ...
        const closest = [...crossings]
            .filter((s) => s !== "0x0")
            .sort(firstBy((c) =>
                grid1.forCoord(new Coord(c)) +
                grid2.forCoord(new Coord(c))
                ))[0];
        return grid1.forCoord(new Coord(closest)) +
            grid2.forCoord(new Coord(closest));
    }
);
(async () => {
